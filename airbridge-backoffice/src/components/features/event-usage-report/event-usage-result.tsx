...
export function EventUsageResult({
  ...
  return (
    ...
    >
      <AccordionItem value={eventId} className="border rounded-lg">
        ...
        <AccordionContent className="border-t">
          ...
          <div className="p-5 px-6 border-b">
            ...
            <p className="text-xs text-muted-foreground mb-3.5 leading-relaxed">
              이 이벤트({eventId})가 Airbridge 리포트에서 조회된 횟수입니다.
              어떤 리포트 유형(actuals, funnel, retention 등)에서 사용되었는지 확인할 수 있습니다.
            </p>
            <ChartContainer
              data={REPORT_USAGE_DATA}
              series={REPORT_USAGE_APIS}
              sql={REPORT_USAGE_SQL}
              chartHeight={180}
              tableDescription={{
                firstCol: "조회일",
                rest: "리포트 API별 조회 횟수",
